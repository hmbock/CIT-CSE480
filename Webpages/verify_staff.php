<!-- start PHP code -->
        <?php
            ...
            if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
                ...
                $search = mysql_query("SELECT staff_email, staff_hash, staff_active FROM Staff WHERE staff_email='".$email."' AND staff_hash='".$hash."' AND staff_active='0'") or die(mysql_error()); 
                $match  = mysql_num_rows($search); //records the number of rows that have matched the search

                if($match > 0){
                    // We have a match, activate the account
                    mysql_query("UPDATE Staff SET staff_active='1' WHERE staff_email='".$email."' AND staff_hash='".$hash."' AND staff_active='0'") or die(mysql_error()); //change active to 1 where the email, hash and field active = 0 match up
...
<html>
    <head>
        <title>Betwixt Booking | Staff Account Verification</title>
		<meta charset="utf-8">
